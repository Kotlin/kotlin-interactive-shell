...
public class TerminalFactory
{
    ...
    public static synchronized Terminal create(String ttyDevice) {
        if (lib.jline.internal.Log.TRACE) {
            //noinspection ThrowableInstanceNeverThrown
            lib.jline.internal.Log.trace(new Throwable("CREATE MARKER"));
        }

        String type  = lib.jline.internal.Configuration.getString(JLINE_TERMINAL);
        if (type == null) {
            type = AUTO;
        ...
        lib.jline.internal.Log.debug("Creating terminal; type=", type);
        ...
        try {
            ...
            if (tmp.equals(UNIX)) {
                ...
                if (tmp.equals(AUTO)) {
                    String os = lib.jline.internal.Configuration.getOsName();
            ...
            lib.jline.internal.Log.error("Failed to construct terminal; falling back to unsupported", e);
            t = new UnsupportedTerminal();
        }

        lib.jline.internal.Log.debug("Created Terminal: ", t);

        try {
            t.init();
        }
        catch (Throwable e) {
            lib.jline.internal.Log.error("Terminal initialization failed; falling back to unsupported", e);
    ...
    public static synchronized void configure(final String type) {
        lib.jline.internal.Preconditions.checkNotNull(type);

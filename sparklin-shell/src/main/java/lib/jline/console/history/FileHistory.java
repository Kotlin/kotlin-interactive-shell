    ...
    implements PersistentHistory, Flushable
{
    ...
    public void load(final File file) throws IOException {
        ...
        if (file.exists()) {
            lib.jline.internal.Log.trace("Loading history from: ", file);
    ...
    public void flush() throws IOException {
        lib.jline.internal.Log.trace("Flushing history");

        if (!file.exists()) {
            File dir = file.getParentFile();
            if (!dir.exists() && !dir.mkdirs()) {
                lib.jline.internal.Log.warn("Failed to create directory: ", dir);
            }
            if (!file.createNewFile()) {
                lib.jline.internal.Log.warn("Failed to create file: ", file);
            }
        }
    ...
    public void purge() throws IOException {
        lib.jline.internal.Log.trace("Purging history");

        clear();

        if (!file.delete()) {
            lib.jline.internal.Log.warn("Failed to delete history file: ", file);
        }
    }
}
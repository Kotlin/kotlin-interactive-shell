...
public class ShutdownHooks
{
    ...
    public static synchronized <T extends Task> T add(final T task) {
        lib.jline.internal.Preconditions.checkNotNull(task);

        // If not enabled ignore
        if (!enabled) {
            lib.jline.internal.Log.debug("Shutdown-hook is disabled; not installing: ", task);
            return task;
        }
        ...
        // Track the task
        lib.jline.internal.Log.debug("Adding shutdown-hook task: ", task);
        tasks.add(task);
    ...
    private static synchronized void runTasks() {
        lib.jline.internal.Log.debug("Running all shutdown-hook tasks");

        // Iterate through copy of tasks list
        for (Task task : tasks.toArray(new Task[tasks.size()])) {
            lib.jline.internal.Log.debug("Running task: ", task);
            try {
                task.run();
            }
            catch (Throwable e) {
                lib.jline.internal.Log.warn("Task failed", e);
            }
        }
    ...
    private static Thread addHook(final Thread thread) {
        lib.jline.internal.Log.debug("Registering shutdown-hook: ", thread);
        try {
            Runtime.getRuntime().addShutdownHook(thread);
        }
        catch (AbstractMethodError e) {
            // JDK 1.3+ only method. Bummer.
            lib.jline.internal.Log.debug("Failed to register shutdown-hook", e);
        }
        return thread;
    }
    ...
    private static void removeHook(final Thread thread) {
        lib.jline.internal.Log.debug("Removing shutdown-hook: ", thread);
        ...
        try {
            ...
            // JDK 1.3+ only method. Bummer.
            lib.jline.internal.Log.debug("Failed to remove shutdown-hook", e);
        }
